...
function removeFirstOccurrences(str, value) {
  ...
  return firstOccurenceIndex >= 0
    ? `${str.slice(0, firstOccurenceIndex)}${str.slice(
        firstOccurenceIndex + value.length
      )}`
    : str;
}
...
function removeLastOccurrences(str, value) {
  const lastOccurenceIndex = str.lastIndexOf(value);
  return lastOccurenceIndex >= 0
    ? `${str.slice(0, lastOccurenceIndex)}${str.slice(
        lastOccurenceIndex + value.length
      )}`
    : str;
}
...
function isPalindrome(str) {
  const preprocessedString = str
    ...
    .replaceAll('?', '')
    .replaceAll(',', '');
  return preprocessedString === preprocessedString.split('').reverse().join('');
}
...
function invertCase(str) {
  ...
  str.split('').forEach((letter) => {
    if (letter === letter.toUpperCase()) {
      invertedArray.push(letter.toLowerCase());
    } else {
      invertedArray.push(letter.toUpperCase());
    }
  });
...
function encodeToRot13(str) {
  ...
  keys.forEach((el, i) => {
    encodingDict[el] = values[i];
  });

  const encodedArray = [];
  str.split('').forEach((letter) => {
    encodedArray.push(encodingDict[letter]);
  });
